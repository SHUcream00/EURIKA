...
def recruit(slice) -> str:
    elite = {'상급엘리트': {'시즈': 6, '샤이닝': 6, '나이팅게일': 6, '이프리트': 6, '엑시아': 6, '실버애쉬': 6,\
    ...
    if elite_token in slice: target_dict = elite
    else: target_dict = pos
    ...
    for i in list(temp)[::-1]:
        ...
        if len(intersect) != 0:
            if elite_token in slice and elite_token not in i:
                print(i)
                continue
